...
describe('glass accessibility fallbacks', () => {
  it('densifies both chrome and readable tints and drops grain in prefers-contrast: more media query', () => {
    ...
    expect(mediaBlock).toMatch(/--glass-grain-display\s*:\s*none/);
  });

  it('densifies both chrome and readable tints and drops grain in data-contrast=more preview selector', () => {
    const previewBlock = extractSelectorBlock(glassCss, ".glass-a11y-preview[data-contrast='more']");
    ...
    expect(previewBlock).toMatch(/--glass-grain-display\s*:\s*none/);
  });

  it('forces opaque background, drops blur and drops grain in prefers-reduced-transparency media query', () => {
  ...
  it('forces opaque background, drops blur and drops grain in data-transparency=reduced preview selector', () => {
