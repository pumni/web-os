...
| --- | --- | --- |
| Floating glass | `GlassSurface variant="panel\|bar\|window\|titlebar"` or the matching `glass-*` utility | Floating layers: dialogs, sheets, popovers, command palette, toast, topbar, dock, sidebar rail, OS windows, pills/overlays. Glassmorphism: frosted vibrant fill + luminous edge pair (`--glass-highlight`/`--glass-shadow-edge`) + inner sheen (`--glass-sheen`), tokenized `--glass-saturate`, directional `--shadow-glass`. **Must float over a colourful backdrop** (ADR-0015): chrome glass over whatever sits behind it; feature/hero glass cards wrapped in 2-blob container. On flat backgrounds → use `Card variant="solid"`. Dense content (forms/tables) always solid. |
