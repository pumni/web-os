...
1. **Token Boundaries**: All colors must use tokens (`var(--surface-*)`, `var(--text-*)`, `var(--border-*)`). Raw `oklch()` or primitive variables (`--indigo-*`) fail design token lint.
...
- [ ] `bun --filter @pumni/ui typecheck` passes.
