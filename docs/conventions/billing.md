...
- **Quota invariant**: Plan-quota checks are atomic in Postgres (using `pg_advisory_xact_lock` + `volatile` functions, e.g. `supabase/migrations/024_atomic_quota_checks.sql`). Never re-implement quota prechecks in app code. See the [Non-Atomic Quota & Concurrency Checks](../ai/common-mistakes.md#3-non-atomic-quota--concurrency-checks) pitfall.
