...
- **Quota invariant**: Plan-quota checks are atomic in Postgres (using `pg_advisory_xact_lock` + `volatile` functions, e.g. `supabase/migrations/023_watch_rooms_limit_rls.sql`). Never re-implement quota prechecks in app code. See [common-mistakes.md](../ai/common-mistakes.md) §15–16.
