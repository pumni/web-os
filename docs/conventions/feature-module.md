...
> [!NOTE]
> Emitting a config object per feature would fall into the override trap (only the last declared feature gets enforced). `check-feature-boundary.mjs` (run during `bun run ai:eval`) automatically verifies that all features are enforced across all three scopes.
