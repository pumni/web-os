...
2. **Prune Meta-Inversion Footprint:** Use the rule-efficacy metric to justify deleting two unproven meta-about-meta files: `context-system.md` and `memory-layer.md`. Compress essential planes/flow/cache details into `AGENTS.md` and fold the hybrid memory layer description into `docs/ai/agent-behavior.md`. Retain `agent-command-policy.md` as the canonical SSOT for PowerShell 7 host environment guidelines.
...
- Unified Win32 + PowerShell 7 command discipline is documented as the host default in `AGENTS.md`, aligning with `agent-command-policy.md`.

**Negative / costs:**
- Removal of two documentation files (their compressed essential sections now reside in `AGENTS.md` and `agent-behavior.md`).
